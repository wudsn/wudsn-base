...
public final class RepositoryValidation {
	...
	public boolean isStringSpecified(Object owner, Attribute attribute, String stringValue) {
		...
		if (StringUtility.isEmpty(stringValue)) {
			...
			messageQueue.sendMessage(owner, attribute, Messages.E300,
	...
	public boolean isStringDefined(Object owner, Attribute attribute, String stringValue, boolean defined) {
		...
		if (StringUtility.isSpecified(stringValue) && !defined) {
			...
			messageQueue.sendMessage(owner, attribute, Messages.E301,
					attribute.getDataType().getLabelWithoutMnemonics(), stringValue);
			return false;
		}
		return true;
	}
	...
	public boolean isStringValid(Object owner, Attribute attribute, String stringValue) {
		...
		if (stringValue.length() > maximumLength) {
			...
			messageQueue.sendMessage(owner, attribute, Messages.E302,
					attribute.getDataType().getLabelWithoutMnemonics(),
					TextUtility.formatAsDecimal(stringValue.length()), TextUtility.formatAsDecimal(maximumLength));
			return false;
		}
		...
		if (allowedCharacters != null) {
			for (int i = 0; i < stringValue.length(); i++) {
				...
				if (allowedCharacters.indexOf(c) < 0) {
					...
					messageQueue.sendMessage(owner, attribute, Messages.E303,
							attribute.getDataType().getLabelWithoutMnemonics(), Character.toString(c),
							TextUtility.formatAsDecimal(i + 1), allowedCharacters);
					return false;
				}
			}
		}
	...
	public boolean isLongValid(Object owner, Attribute attribute, long minimumValue, long maximumValue,
			long longValue) {
		if (attribute == null) {
			throw new IllegalArgumentException("Parameter 'attribute' must not be null.");
		}
		if (longValue < minimumValue || longValue > maximumValue) {
			...
			messageQueue.sendMessage(owner, attribute, Messages.E304,
					attribute.getDataType().getLabelWithoutMnemonics(), TextUtility.formatAsDecimal(longValue),
					TextUtility.formatAsDecimal(minimumValue), TextUtility.formatAsDecimal(maximumValue));
			return false;
		}
		return true;
	}
	...
	public boolean isMemorySizeValid(Object owner, Attribute attribute, long minimumValue, long maximumValue,
		...
		if (longValue < minimumValue || longValue > maximumValue) {
			// ERROR: {0} has value {1} and is not between {2} and {3}.
			messageQueue.sendMessage(owner, attribute, Messages.E304,
					attribute.getDataType().getLabelWithoutMnemonics(), TextUtility.formatAsMemorySize(longValue),
					TextUtility.formatAsDecimal(minimumValue), TextUtility.formatAsMemorySize(maximumValue));
			return false;
		}
		return true;
	}
