...
public final class HexUtility {
	...
	/**
	 * Gets the hex string for a single byte value. Letters are in upper case.
	 * 
	 * @param byteValue
	 *            The byte value.
	...
	public static int getLongValueHexLength(long longValue) {
		if (longValue < 0) {
			throw new RuntimeException(
					"Parameter 'longValue' must not be negative. Specified value is " + longValue + ".");
		}
	...
	/**
	 * Gets the hex string for a long value. Letters are in upper case.
	 * 
	 * @param longValue
	 *            The non-negative long value.
	 * @return The string, not empty and not <code>null</code>.
	 ...
	 *            The non-negative long value.
	 * @param length
	 *            The minimum number of characters to be used. If the result string
	 *            is shorter, spaces will be prepended in case a length other then
	 *            {@link #AUTOMATIC_LENGTH} is specified.
	 * @return The string, not empty and not <code>null</code>.
	 */
	public static String getLongValueHexString(long longValue, int length) {
		...
		if (length < 0) {
			throw new IllegalArgumentException(
					"Parameter 'length' must not be negative. Specified value is " + length + ".");
		}
