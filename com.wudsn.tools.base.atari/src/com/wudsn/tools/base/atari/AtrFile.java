...
public final class AtrFile {
	...
	public static final class DirectoryEntry {
		 ...
		 * @param dosFileName
		 *            The file in "8.3" notation, i.e. 1..12 characters, trimmed, with
		 *            ".", not <code>null</code>.
		 * @param startSector
		 */
		...
		public String toString() {
			return "directoryFileName=" + directoryFileName + ", dosFileName=" + dosFileName + " startSector="
					+ startSector;
		}
	...
	public static final int BOOT_SECTORS_SIZE_SD = BOOT_SECTORS * SECTOR_SIZE_SD;
	...
	private AtrFile(byte[] data) {
		...
		if (data.length >= BOOT_SECTORS_SIZE_SD) {
			sectorCount = (data.length - BOOT_SECTORS_SIZE_SD) / sectorSize + BOOT_SECTORS;
		} else {
		...
		if (sectorCount > vtocSector) {
			...
			case 2:
				...
				break;
			default:
				if (startsWith(bootSectorOffset, new byte[] { 0x01, 0x09, 0x00, 0x32, 0x06, 0x32 })) {
					dosVersion = DosVersion.ATARI_DOS_30;
				} else if (startsWith(bootSectorOffset, new byte[] { 0x4d, 0x03, 0x00, 0x07 })) {
					dosVersion = DosVersion.MYDOS;
				} else if (startsWith(bootSectorOffset,
						new byte[] { 0x00, 0x03, 0x00, 0x07, 0x40, 0x07, 0x4c, (byte) 0x80, 0x07 })) {
					dosVersion = DosVersion.SPARTADOS;
				} else if (startsWith(bootSectorOffset,
						new byte[] { 0x00, 0x03, 0x00, 0x030, (byte) 0xe0, 0x07, 0x4c, (byte) 0x80, 0x30 })) {
	...
	/**
	 * Creates an ATR file header. The following data sizes are used according to
	 * SIO2PC readme:
	 * <ul>
	 ...
	 * 
	 * @param dataSize
	 *            The content length. A positive integer which must be a multiple of
	 *            the sector size and a multiple of 16.
	 * @param sectorSize
	...
	public static byte[] createHeader(int dataSize, int sectorSize) {
		...
		if (sectorSize < 128) {
			throw new IllegalArgumentException(
					"Parameter 'sectorSize' must not be less than 128, actual value was " + sectorSize + ".");
		}
		if (sectorSize > 65535) {
			throw new IllegalArgumentException(
					"Parameter 'sectorSize' must not be largen than 65535, actual value was " + sectorSize + ".");
		}

		// Round everything except the boot sectors up to full sectors.
		if (sectorSize == SECTOR_SIZE_DD && dataSize >= BOOT_SECTORS_SIZE_SD) {
			dataSize = ((dataSize - BOOT_SECTORS_SIZE_SD + sectorSize - 1) / sectorSize) * sectorSize
					+ BOOT_SECTORS_SIZE_SD;
		} else {
			dataSize = ((dataSize + sectorSize - 1) / sectorSize) * sectorSize;
		}
		if ((dataSize % PARAGRAPH_SIZE) != 0) {
			throw new IllegalArgumentException("Parameter 'content' has effective size " + dataSize
					+ " which is not a multiple of the paragraph size " + PARAGRAPH_SIZE + ".");
		}
	 ...
	 *         ATR magic bytes, <code>false</code> otherwise.
	 */
	public static boolean isHeader(byte[] data) {
		if (data == null) {
			throw new IllegalArgumentException("Parameter 'data' must not be null.");
		}
		if (data.length >= 16 && data[0] == (byte) 0x96 && data[1] == (byte) 0x02) {
			return true;
		}
		return false;
	}
	...
	public int getSectorSize(int sectorNumber) throws AtrException {
		if (sectorNumber <= 0) {
			throw new AtrException(
					"Parameter 'sectorNumber' must be positive. Specified value is " + sectorNumber + ".");
		}
		if (sectorNumber > sectorCount) {
			throw new AtrException("Parameter 'sectorNumber' must be less than the maximum sector " + sectorCount
					+ ". Specified value is " + sectorNumber + ".");
		}
		int sectorSize;
	...
	public int getSectorStartOffset(int sectorNumber) throws AtrException {
		...
		if (sectorNumber <= BOOT_SECTORS) {
			...
			offset += (sectorNumber - 1 - BOOT_SECTORS) * sectorSize + BOOT_SECTORS_SIZE_SD;
		}
		return offset;
	}
	...
	public byte[] getSectors(int startSectorNumber, int endSectorNumber, int sectorSize) throws AtrException {
		if (startSectorNumber <= 0) {
			throw new AtrException(
					"Parameter 'sectorNumber' must be positive. Specified value is " + startSectorNumber + ".");
		}
		if (startSectorNumber > sectorCount) {
			throw new AtrException("Parameter 'sectorNumber' must be less than the maximum sector " + sectorCount
					+ ". Specified value is " + startSectorNumber + ".");
		}
		if (endSectorNumber <= 0) {
			throw new AtrException(
					"Parameter 'endSectorNumber' must be positive. Specified value is " + endSectorNumber + ".");
		}
		if (endSectorNumber > sectorCount) {
			throw new AtrException("Parameter 'sectorNumber' must be less than the maximum sector " + sectorCount
					+ ". Specified value is " + endSectorNumber + ".");
		}
	 ...
	 * @param usedSectors
	 *            The list of sector numbers for used sectors or <code>null</code> .
	 * @return The binary content of the file or <code>null</code> if the file does
	 *         not exist.
	 * @throws AtrException
	...
	private List<DirectoryEntry> getDirectory(String fileNameFilter) throws AtrException {
		...
		if (!hasDirectory()) {
			throw new AtrException("Cannot get directory of unknown or unsupported DOS version " + dosVersion + ".");
		}
		...
		while (!complete) {
			...
			// of the density.
			int relevantSectorSize = Math.min(AtrFile.SECTOR_SIZE_SD, sectorSize);
			int offset = 0;
			while (offset < relevantSectorSize && !complete) {
				...
				if (status == 0x00) {
					...
					if ((status & 0x80) == 0x00 && (status & 0x40) != 0x00) {
						...
						String dosFileName = fileName.trim() + "." + extension.trim();
						...
						if (StringUtility.isEmpty(fileNameFilter) || dosFileName.equals(fileNameFilter)) {
							...
							DirectoryEntry directoryEntry = new DirectoryEntry(fileName + extension, dosFileName,
									startSector);
	...
	@Override
	public String toString() {
		return "sectorSize=" + sectorSize + ", sectorCount=" + sectorCount + ", DOS=" + dosVersion;
	}
}
